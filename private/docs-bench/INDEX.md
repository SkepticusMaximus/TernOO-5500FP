...
| `decisions/2026-09-19-merge-back-ledger.md` | THE documentation-phase to-do list (CAI, 19/09). The whitepaper canonical is still June v0.4 while `docs/site` publicly declares the repo canonical — the standing hazard. Lists corrections owed back from the ASPLOS submission, fork-only improved wordings, cuts that must NOT propagate, and three new entries: Structural Emergence (the unifying principle), TDA's status as a working note with an undefended packing flank, and the theoretical collective — SQG as established foundation underwriting three hypotheses (TDA, SE, MRS). Ends with the captain's phase order. |
