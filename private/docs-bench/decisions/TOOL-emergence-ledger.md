...
| 7 | Captain's clarification (23-09 transcript) | SELF-AUDIT CATCH: the click-signal channel (.sig TSV lines) is a mini-language IN THE GAPS — by the one-truth law, face events must themselves be WORDS (an event stream the engine reads), not a side protocol. The wire already speaks word-deltas one way; it must speak word-events the other way. |
