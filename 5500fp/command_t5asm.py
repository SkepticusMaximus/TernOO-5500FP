...
# String/list-runtime syscalls (Runtime Value Substrate). Handles are heap addrs.
_SYS_STR_FROMBUF = 41
_SYS_STR_LEN     = 42
_SYS_STR_UPPER   = 49
_SYS_STR_LOWER   = 50
...
_SYS_LIST_SET    = 63
_SYS_LIST_JOIN   = 65
_SYS_LIST_REVERSE = 66
_SYS_LIST_SORT   = 67
_SYS_LIST_UNIQUE = 68
_PIG_DIALOG_CHOICE_LIST = 117
...
_OUTPUT_KIND = {
    ...
    'cmd_text_split': 'list', 'cmd_text_join': 'text', 'cmd_text_format': 'text',
    'cmd_list_count': 'number', 'cmd_list_first': 'text', 'cmd_list_last': 'text',
    'cmd_list_reverse': 'list', 'cmd_list_sort': 'list', 'cmd_list_unique': 'list',
    'cmd_ctl_repeat': 'list',
    'cmd_io_prompt': 'text', 'cmd_io_display': 'none',
    'cmd_io_confirm': 'number', 'cmd_io_choice': 'text',
}

# Params whose value is a list handle (drives _load_list vs _load_text/_num).
_LIST_PARAMS = {
    'cmd_text_join': {'list'}, 'cmd_text_format': {'args'},
    'cmd_list_count': {'list'}, 'cmd_list_first': {'list'},
    'cmd_list_last': {'list'}, 'cmd_list_reverse': {'list'},
    'cmd_list_sort': {'list'}, 'cmd_list_unique': {'list'},
    'cmd_io_choice': {'options'},
}


def list_params(name: str) -> set:
    return _LIST_PARAMS.get(name, set())
...
_TEXT_BUF_PARAMS = {
    ...
    'cmd_text_split': {'text', 'delimiter'},
    'cmd_text_join': {'separator'}, 'cmd_text_format': {'template'},
    'cmd_ctl_repeat': {'value'},
    'cmd_io_prompt': {'message'}, 'cmd_io_confirm': {'message'},
    'cmd_io_display': {'value', 'title'}, 'cmd_io_choice': {'prompt'},
}
...
class _Ctx:
    ...
    def __init__(self, out_text: str = None):
        ...
        self.needs_comma = False     # a _comma_str literal is referenced
...
def _load_list(arg, reg: int, ctx) -> list:
    """Emit code leaving a LIST HANDLE in R<reg>.  Clobbers R<reg+1> for a
    literal (a comma-separated string split into a list)."""
    kind = arg[0]
    if kind == 'handleslot':
        return [f'    LI   R{reg}, {arg[1]}',
                f'    LDW  R{reg}, R{reg}, 0']
    if kind == 'listlit':
        ...
        return [
            f'    LI   R2, {arg[1]}',
            f'    LI   R1, {_SYS_STR_FROMBUF}', '    SYSCALL', f'    MOV  R{reg}, R1',
            '    LI   R2, _comma_str',
            f'    LI   R1, {_SYS_STR_FROMBUF}', '    SYSCALL', f'    MOV  R{t2}, R1',
            f'    MOV  R2, R{reg}', f'    MOV  R3, R{t2}',
            f'    LI   R1, {_SYS_STR_SPLIT}', '    SYSCALL', f'    MOV  R{reg}, R1',
        ]
    raise CommandCompileError(f'list argument required, got {arg!r}')
...
def _cmd_io_choice(args, dst, ctx):
    """choice(options, prompt) -> selected element handle (0 = cancel)."""
    lines = _load_list(args[0], dst, ctx)
    lines += _load_text(args[1], dst + 1)
    lines += [
        f'    MOV  R2, R{dst}',
        f'    MOV  R3, R{dst + 1}',
        '    ADDI R3, R3, 1                ; prompt chars',
        f'    LI   R1, {_PIG_DIALOG_CHOICE_LIST}',
        '    SYSCALL',
        f'    MOV  R{dst}, R1',
    ]
    return lines
...
# ---------------------------------------------------------------------------
# List commands (fixed-length heap lists)
# ---------------------------------------------------------------------------

def _cmd_text_split(args, dst, ctx):
    """split(text, delimiter) -> list of string handles."""
    lines = _load_text(args[0], dst)
    lines += _load_text(args[1], dst + 1)
    lines += [f'    MOV  R2, R{dst}', f'    MOV  R3, R{dst + 1}',
              f'    LI   R1, {_SYS_STR_SPLIT}', '    SYSCALL', f'    MOV  R{dst}, R1']
    return lines


def _cmd_text_join(args, dst, ctx):
    """join(list, separator) -> text."""
    lines = _load_list(args[0], dst, ctx)
    lines += _load_text(args[1], dst + 1)
    lines += [f'    MOV  R2, R{dst}', f'    MOV  R3, R{dst + 1}',
              f'    LI   R1, {_SYS_LIST_JOIN}', '    SYSCALL', f'    MOV  R{dst}, R1']
    return lines


def _cmd_text_format(args, dst, ctx):
    """format(template, args-list) -> text ({0},{1},... substitution)."""
    lines = _load_text(args[0], dst)
    lines += _load_list(args[1], dst + 1, ctx)
    lines += [f'    MOV  R2, R{dst}', f'    MOV  R3, R{dst + 1}',
              f'    LI   R1, {_SYS_STR_FORMAT}', '    SYSCALL', f'    MOV  R{dst}, R1']
    return lines


def _cmd_list_count(args, dst, ctx):
    lines = _load_list(args[0], dst, ctx)
    lines += [f'    MOV  R2, R{dst}', f'    LI   R1, {_SYS_LIST_LEN}',
              '    SYSCALL', f'    MOV  R{dst}, R1']
    return lines


def _cmd_list_first(args, dst, ctx):
    lines = _load_list(args[0], dst, ctx)
    lines += [f'    MOV  R2, R{dst}', '    LI   R3, 0',
              f'    LI   R1, {_SYS_LIST_GET}', '    SYSCALL', f'    MOV  R{dst}, R1']
    return lines


def _cmd_list_last(args, dst, ctx):
    n = dst + 1
    lines = _load_list(args[0], dst, ctx)
    lines += [
        f'    MOV  R2, R{dst}', f'    LI   R1, {_SYS_LIST_LEN}', '    SYSCALL',
        f'    MOV  R{n}, R1', f'    SUBI R{n}, R{n}, 1',
        f'    MOV  R2, R{dst}', f'    MOV  R3, R{n}',
        f'    LI   R1, {_SYS_LIST_GET}', '    SYSCALL', f'    MOV  R{dst}, R1',
    ]
    return lines


def _list_transform(sysnum, has_flag=False):
    def emit(args, dst, ctx):
        lines = _load_list(args[0], dst, ctx)
        setup = [f'    MOV  R2, R{dst}']
        if has_flag:                       # list_sort: ascending flag
            asc = 1 if (len(args) < 2 or args[1][0] != 'num' or args[1][1]) else 0
            setup.append(f'    LI   R3, {asc}')
        setup += [f'    LI   R1, {sysnum}', '    SYSCALL', f'    MOV  R{dst}, R1']
        return lines + setup
    return emit


def _cmd_ctl_repeat(args, dst, ctx):
    """repeat(count, value) -> list of `count` copies of value."""
    cnt, val, idx = dst + 1, dst + 2, dst + 3
    lines = _load_num(args[0], cnt)
    lines += _load_text(args[1], val)
    lines += [f'    MOV  R2, R{cnt}', f'    LI   R1, {_SYS_LIST_ALLOC}',
              '    SYSCALL', f'    MOV  R{dst}, R1']
    loop, done = ctx.label('rep'), ctx.label('repd')
    lines += [
        f'    LI   R{idx}, 0',
        f'{loop}:',
        f'    BLT  R{idx}, R{cnt}, {loop}_b',
        f'    JMP  {done}',
        f'{loop}_b:',
        f'    MOV  R2, R{dst}', f'    MOV  R3, R{idx}', f'    MOV  R4, R{val}',
        f'    LI   R1, {_SYS_LIST_SET}', '    SYSCALL',
        f'    ADDI R{idx}, R{idx}, 1',
        f'    JMP  {loop}',
        f'{done}:',
    ]
    return lines
...
_DISPATCH = {
    ...
    'cmd_text_split':    _cmd_text_split,
    'cmd_text_join':     _cmd_text_join,
    'cmd_text_format':   _cmd_text_format,
    'cmd_list_count':    _cmd_list_count,
    'cmd_list_first':    _cmd_list_first,
    'cmd_list_last':     _cmd_list_last,
    'cmd_list_reverse':  _list_transform(_SYS_LIST_REVERSE),
    'cmd_list_sort':     _list_transform(_SYS_LIST_SORT, has_flag=True),
    'cmd_list_unique':   _list_transform(_SYS_LIST_UNIQUE),
    'cmd_ctl_repeat':    _cmd_ctl_repeat,
    'cmd_io_prompt':     _cmd_io_prompt,
    'cmd_io_display':    _cmd_io_display,
    'cmd_io_confirm':    _cmd_io_confirm,
    'cmd_io_choice':     _cmd_io_choice,
}
